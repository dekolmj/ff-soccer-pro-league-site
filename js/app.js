...
function area(tab,body){
  ...
  return '<div class="wrap"><div class="area"><nav class="side"><div class="me"><div class="avatar" style="width:52px;height:52px;border:2px solid #0B0B0A"></div><b>'+esc(p.n).replace(' ','<br>')+'</b><span>'+POSN[p.pos]+' · '+tn(p.team)+' · Nº '+p.num+'</span></div>'+
...
P.entrar=function(){
  ...
  '<div class="demo"><div class="lbl" style="color:var(--gold)">Protótipo · atleta de demonstração</div><div class="row"><div class="avatar" style="width:42px;height:42px"></div><div><b style="font-family:var(--cond);text-transform:uppercase;font-size:17px">Marcelo Tavares</b><div class="lbl">Meia · Falcões · Nº 10</div></div></div><button class="btn primary sm" id="demoBtn" type="button" style="justify-content:center">Entrar como Marcelo</button></div>'+
...
P['minha-area-inscricao']=function(){
  ...
  '<div class="grid2"><div style="display:grid;gap:16px;align-content:start">'+
   ...
   '<div class="card" style="display:grid;gap:12px;border-color:var(--gold2)"><div class="between"><h3>2ª Temporada</h3><span class="chip gold">🔒 Prioridade garantida</span></div><p class="muted" style="margin:0">Como você joga a 1ª Temporada, tem prioridade na renovação. A janela abre em 20/05 e novas vagas só abrem se sobrarem.</p>'+
  ...
  '</div><div style="display:grid;gap:16px;align-content:start">'+
   ...
   '<div class="card"><div class="between" style="margin-bottom:8px"><h3>Meus dados</h3><button class="btn sm" id="editBtn">Atualizar</button></div><div class="kv"><div class="between"><span>E-mail</span><span>marcelo.t•••@email.com</span></div><div class="between"><span>Celular</span><span>(11) 9••••-4821</span></div><div class="between"><span>Unidade FF</span><span>'+p.unit+'</span></div><div class="between"><span>Posição</span><span>'+POSN[p.pos]+'</span></div><div class="between"><span>Camisa</span><span>'+esc(p.n.split(' ').slice(-1)[0].toUpperCase())+' · Nº '+p.num+'</span></div></div></div>'+
...
function bind(page){
  ...
  if(page==='entrar'){
    document.getElementById('demoBtn').addEventListener('click',function(){login('falcoes-10');toast('Bem-vindo, Marcelo!');});
